...
string ENRICtoStringSVG (ENRICdevice * device, bool quote){
    ...
    //Making the arrows fot the quotes of the device
    deviceSVG += ENRICarrowMarkerSVG(); 
    deviceSVG += "\n"; 

    //Building the cam
        //Internal circle of the cam: I will only need a section of the circle, so I just draw an arc
    ...
    //Valve
        //First rectangle componing the valve
            ...
            if(device->Alpha >= 0 && device->Alpha < PI){
                ValveStartY = device->rMin;
            }
            else if(device->Alpha >= PI && device->Alpha < (1.5*PI-device->Gamma)){
            ...
            else if (device->Alpha >= (1.5*PI-device->Gamma) && device->Alpha < (1.5*PI+device->Gamma)){
                ValveStartY = device->rMax;
            }
            else if (device->Alpha >= (1.5*PI+device->Gamma) && device->Alpha < 2*PI){
    ...
    //First rectangle componing the valve            
    deviceSVG += "<rect x=\"" + to_string(xC-(device->lenValve)/20) + "\" y=\"" + to_string(yC+ValveStartY) + "\" "; //defining starting point
    ...
    deviceSVG += "<rect x=\"" + to_string(xC-(device->diamValve)/2) + "\" y=\"" + to_string(yC+device->lenValve+ValveStartY) + "\" "; //defining starting point
    ...
    if(quote){
        ...
        if((fmod(device->Alpha, PI) > PI/2)&&(fmod(device->Alpha, PI) < 3*PI/2)){ // the quote is on the right side
            deviceSVG += ENRICquoteDistSVG(xC+device->lenValve/20, yC+ValveStartY,xC+device->lenValve/20 , yC+ValveStartY+device->lenValve, device->lenValve/10+distQuote, lQuote, false);
        }
        else{
            deviceSVG += ENRICquoteDistSVG(xC-device->lenValve/20, yC+ValveStartY,xC-device->lenValve/20, yC+ValveStartY+device->lenValve, device->lenValve/10+distQuote, lQuote, true);
        }

        // Quote of the diameter of the valve
        deviceSVG += ENRICquoteDistSVG(xC-device->diamValve/2, yC+device->lenValve+ValveStartY+device->lenValve/10, xC+device->diamValve/2, yC+device->lenValve+ValveStartY+device->lenValve/10, device->lenValve/7+distQuote, lQuote, true);

        // Quote of the external circle of the cam
        deviceSVG += ENRICquoteDistSVG(xC-device->rMax, yC-device->rMax, xC+device->rMax, yC-device->rMax, device->lenValve/7+distQuote, lQuote, false);
